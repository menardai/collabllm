#!/usr/bin/env python3
"""
Memory-efficient parallel multiturn inference with per-example metric collection using unsloth.

Example run:
Inference for the base model:
On medium dataset:
-------
CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 torchrun --nproc_per_node=8 --master_port=56900 -m scripts.engine.inference_unsloth \
    --dataset_name medium \
...
-------
CUDA_VISIBLE_DEVICES=6,7 torchrun --nproc_per_node=2 --master_port=56900 -m scripts.engine.inference_unsloth \
...
def load_model_and_tokenizer(
    ...
    try:
        ...
        base, tok = FastLanguageModel.from_pretrained(
            ...
            load_in_4bit=bnb_cfg is not None,  # Use 4-bit if bnb_cfg is provided
            device_map={"": device},
        )
        ...
        logger.error(f"Failed to load PeftConfig for {model_name}: {e}, loading as base model.")
        print(f"Attempting to load directly as model: {model_name}")
