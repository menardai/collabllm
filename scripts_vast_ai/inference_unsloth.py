#!/usr/bin/env python3
"""
Memory-efficient inference script for CollabLLM finetuned models using unsloth.
Adapted from notebook_tutorials/inference_finetuned_model.ipynb

pip install unsloth torch transformers peft accelerate safetensors bitsandbytes

Example usage:
python inference_unsloth.py --lora_adapter_path ../checkpoints/llama-3b-checkpoint --task_desc "Recommend a movie." --prompt "Find a film suitable for a date night."
"""
...
def load_model_and_tokenizer(lora_adapter_path, max_seq_length=4096):
    """Load the base model and LoRA adapter using unsloth for memory efficiency."""
    ...
    try:
        ...
        print(f"Base model for LoRA: {base_model_name}")
        
        # Load base model using unsloth FastLanguageModel
        ...
        # If loading as LoRA fails, try loading directly as a model
        print(f"Failed to load as LoRA adapter: {e}")
        print(f"Attempting to load directly as model: {lora_adapter_path}")
    ...
    # Enable native inference mode for better performance
...
def main():
    ...
    model, tokenizer = load_model_and_tokenizer(args.lora_adapter_path, args.max_seq_length)
